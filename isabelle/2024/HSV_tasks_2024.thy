...
| "only_nands (TRUE) = True"  (*Constants are also valid in only_nands*)
| "only_nands (FALSE) = True"
| "only_nands _ = False" (*Anything that isn't NAND, INPUT, or a constant is invalid*)
...
next
  ...
  have "x mod 10 < 10"
    by simp
  hence "a < 10" 
    by (metis digits10.elims list.simps(1) local.Cons(2) mod_less_divisor zero_less_numeral)
  hence "\<forall>d \<in> set ds. d < 10" 
    by (simp add: Cons.hyps)
  then show ?case
    by (simp add: \<open>a < 10\<close>)
qed 

text \<open> Every digit is less than 10. \<close>
corollary 
  "\<forall>d \<in> set (digits10 n). d < 10" 
  by (simp add: digits10_all_below_10_helper)

text \<open> digits10 never returns an empty list \<close>
lemma digits10_never_empty:
  "ds = digits10 n \<Longrightarrow> length ds > 0"
proof(induct n)
  case 0
  then show ?case
    by simp
  ...
  proof(cases "n > 9")
    ...
    proof(cases "n > 10")
      ...
      have "digits10 n = n mod 10 # (digits10 (n div 10))"
        by (meson True digits10.simps less_imp_le_nat linorder_not_le)
      then show ?thesis by auto
    next
      case False
      have "n = 10" 
        using False True by linarith
      hence "digits10 n = [0,1]"
        by simp
      hence "sum10 [0,1] = 10"
        by simp
...
theorem "\<forall>A B::nat. (A \<ge> 0 \<and> A < 10 \<and> B \<ge> 0 \<and> B < 10) 
         \<Longrightarrow> sum10([B,A,B,A,B,A]) dvd 37"
  by auto
